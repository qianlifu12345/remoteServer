...
# 优先根据tag生成版本号，没有tag时取分支branch
version=`git describe --tags 2>/dev/null || git rev-parse --abbrev-ref HEAD`
...
# 正式环境配置
context_prod="prod"
registry_prod="reg.maizuo.com"
project_prod="data"

# 新VPC集群正式环境配置
context_vpc="vpc"
registry_vpc="reg.maizuo.com"
project_vpc="data"
...
以下选项适用指令（docker，deploy，all）
       ...
       -r [registry]   指定镜像host选项，例如：[./make all -r reg.miz.so]
...
_chgimg(){
	...
	len=${#arr[@]};
	_version=${arr[$len-1]}
	echo "k8s labels version is :$_version"
	if [ "$context"x = "prod"x ] || [ "$context"x = "vpc"x ];then
		sed -i "s#$_version#$version#" k8s/prod/deployment.yaml
	else
		sed -i "s#$_version#$version#" k8s/dev/deployment.yaml
	fi
...
#测试
_test(){
	_version=`grep 'version' k8s/dev/deployment.yaml | sed -n '1p;1q'`
	IFS="v";
	arr=($_version)
	len=${#arr[@]};
	echo $len
